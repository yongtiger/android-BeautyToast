package cc.brainbook.android.beautytoast.view;
...
import cc.brainbook.android.beautytoast.BeautyToastUtils;
...
//    public static final int STYLE_RECTANGLE = 2;
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (radius == BeautyToast.RADIUS_HALF_OF_HEIGHT) {
            int realRadius = height / 2;
            refreshRadius(realRadius);
            BeautyToastUtils.setGradientDrawable(this, radiusArr, tintColor);
        } else {
            if (radius < 0){
                throw new IllegalArgumentException("radius can not be" +
                        " < 0 but -1(BeautyToast.RADIUS_HALF_OF_HEIGHT)");
            }
